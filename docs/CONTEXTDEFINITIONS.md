...
> v1.0.0 | Part of Dynamic Revenue Orchestrator (DRO) | Industries Common Resources
...
3. Setup → Context Service → Context Service Settings → Toggle on Context Definitions
...
3. Create Integration node (if BRE needed) with fixed attributes: `Id`, `IntegrationProviderDefId`, `RelatedObject`, `RunAsUserId`, `Status`
...
Contains • Equals • Greater Than • Greater Than or Equal • In • Is Empty • Is Not Empty • Less Than • Less Than or Equal • Not Contains • Not Equals • Not In
...
- Cannot persist compound fields (e.g., Contact Name) - must persist individual fields (Salutation, FirstName, LastName)
...
| Definition | Purpose |
|------------|---------|
| AssetContext__stdctx | Asset-based config rules, Constraint Rules Engine |
