  ...
  const fallbackCrawler = new PlaywrightCrawler({
    ...
    async requestHandler({
      ...
      try {
        ...
        await new Promise((resolve) => setTimeout(resolve, coveoWaitMs));
      } catch {
...
async function crawlSalesforcePage(url: Readonly<string>): Promise<string> {
  ...
  const crawler = new PlaywrightCrawler({
    ...
    async requestHandler({
      ...
      try {
        ...
        await new Promise((resolve) => setTimeout(resolve, cookieBannerWaitMs));
      ...
      if (contentFound) {
        ...
        try {
          if (!page.isClosed()) {
            ...
            await Promise.race([
                ...
                .waitForFunction(
                  (): boolean => {
                    ...
                    if (main !== null) {
                      ...
                      const { textContent: textContentMainValue } = clone;
                      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- textContent can be null
                      const text = (textContentMainValue ?? '').trim();
              ...
              new Promise((resolve) => setTimeout(resolve, longWaitMs)),
            ]);
          }
        } catch {
          // Continue
        ...
        await new Promise((resolve) => setTimeout(resolve, contentWaitMs));
      } else {
      ...
      try {
        ...
        const evalResult = await Promise.race([
          page.evaluate(() => {
            ...
            if (bodyContent) {
              // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- cloneNode returns Node, but we know it's an Element
              const clone = bodyContent.cloneNode(true) as Element;
              removeElements(
                clone,
                'script, style, noscript, iframe, svg, canvas, code, pre'
              );
  ...
  try {
    ...
    throw new Error(`Failed to crawl Salesforce page ${url}: ${errorMessage}`);
  }
}
