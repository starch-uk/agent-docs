...
This plan documents the versioning system for docs files and the project as a whole.

## Overview

Every file in the `docs/` directory should be versioned using semantic versioning (semver). The project as a whole also maintains a version in `package.json`.

## Versioning Rules

Versions are compared against the latest commit in the `main` branch. Changes are categorized as:
...
**Example:** Fixing a typo in an API description, correcting a code example, clarifying a configuration option.
...
**Example:** Adding a new "Advanced Patterns" section, documenting a new API method, adding new configuration properties.
...
**Example:** Removing a deprecated API section, restructuring the entire Architecture section, removing support for a major feature.

## Project Versioning

The project as a whole maintains a version in `package.json` (starting at `0.0.0`).

### Version Bump Rules

Changes to docs files should increment the project version based on the **greatest change** in any docs file:
...
Project version: minor bump (because ESLINT.md had a minor bump, which is greater than PMD.md's patch bump)

## Initializing Existing Docs

The existing documentation files in the `docs/` directory need to be initialized with version information when the versioning system is first implemented:
...
All existing docs should be initialized with version `1.0.0` (or appropriate version based on their current state) when the versioning system is first implemented. These existing docs will be tracked going forward using the same semver system.
...
1. **Always compare against main** - Use latest commit in `main` branch as baseline
...
7. **Version on merge** - Determine versions when merging to `main`, not on feature branches
